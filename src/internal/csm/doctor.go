...
func FixLibV8WithContext(ctx context.Context, server int) (string, error) {
	...
	if err := validateMasterInstall(ctx, &buf, user, masterDir); err != nil {
	...
	for _, n := range servers {
		...
		// 3) Sanity check: the files the server needs to boot are there.
		root := filepath.Join(serverDir, "game")
		if missing := missingServerLibs(root); len(missing) > 0 {
			fmt.Fprintf(&buf, "[!] server-%d is still missing %s\n", n, strings.Join(missing, ", "))
			fmt.Fprintf(&buf, "    Tip: run `ldd %s` and look for \"not found\" lines.\n",
				filepath.Join(root, "csgo", "bin", "linuxsteamrt64", "libserver.so"),
			)
			return buf.String(), fmt.Errorf("server-%d still missing %s after validate+sync", n, strings.Join(missing, ", "))
		}

		fmt.Fprintf(&buf, "[✓] server-%d synced; cs2, libserver.so and libv8.so present\n\n", n)
	}
